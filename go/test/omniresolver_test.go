...
import (
	...
	omni "github.com/voxgig-sdk/dietlyapi-integration-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DietlyapiIntegrationError); is {
				out := map[string]any{
					"name":    "DietlyapiIntegrationError",
...
func omniCtx(arg any, client *sdk.DietlyapiIntegrationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DietlyapiIntegrationSDK); is {
			client = live
