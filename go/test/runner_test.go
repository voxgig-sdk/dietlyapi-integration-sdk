...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("DIETLYAPIINTEGRATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("DIETLYAPIINTEGRATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("DIETLYAPIINTEGRATION_TEST_EXPLAIN"); explain != "" {
