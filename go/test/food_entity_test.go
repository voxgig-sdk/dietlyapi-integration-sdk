...
func TestFoodEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DIETLYAPIINTEGRATION_TEST_FOOD_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		foodRef01DataDt0LoadResult := core.ToMapAny(foodRef01DataDt0Loaded)
...
func foodBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DIETLYAPIINTEGRATION_TEST_FOOD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DIETLYAPIINTEGRATION_TEST_FOOD_ENTID"])
