...
func TestBarcodeDirect(t *testing.T) {
	t.Run("direct-load-barcode", func(t *testing.T) {
		setup := barcodeDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-barcode", _mode); _shouldSkip {
			if _reason == "" {
...
type barcodeDirectSetupResult struct {
...
func barcodeDirectSetup(mockres any) *barcodeDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &barcodeDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &barcodeDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
