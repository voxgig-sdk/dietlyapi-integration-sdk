...
func TestBarcodeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		barcodeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.barcode")))
		...
		barcodeRef01DataDt0LoadResult := core.ToMapAny(entityData(barcodeRef01DataDt0Loaded))
...
func barcodeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DIETLYAPI_INTEGRATION_TEST_BARCODE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DIETLYAPI_INTEGRATION_TEST_BARCODE_ENTID"])
