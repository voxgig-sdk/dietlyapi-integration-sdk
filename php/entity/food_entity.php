...
// DietlyapiIntegration SDK Food entity
...
class FoodEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new FoodEntity($this->_client, $opts);
    }

    /**
     * @param Food|array $args Food data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DietlyapiIntegrationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Food|array The current Food data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Food fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DietlyapiIntegrationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Food fields).
     */
     ...
     * Load a single Food.
     *
     * @param FoodLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed FoodLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Food|array The loaded Food as an assoc-array at the
     *   SDK boundary; throws DietlyapiIntegrationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DietlyapiIntegrationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Food items matching the given filter.
     *
     * @param FoodListMatch|array|null $reqmatch Match filter (any subset
     *   of Food fields) as an assoc-array; FoodListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Food[]|array A list of Food items as assoc-arrays at
     *   the SDK boundary; throws DietlyapiIntegrationError on failure (item-5 convention).
     */
