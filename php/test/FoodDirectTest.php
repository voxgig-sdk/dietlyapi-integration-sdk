...
class FoodDirectTest extends TestCase
{
    public function test_direct_list_food(): void
    {
        $setup = food_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-food", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_food(): void
    {
        $setup = food_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-food", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
...
function food_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new DietlyapiIntegrationSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
