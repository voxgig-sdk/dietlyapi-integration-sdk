// Command DietlyapiIntegration-cli is a CLI + REPL for the DietlyapiIntegration SDK,
...
const prompt = "dietlyapi-integration"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "barcode food meta popular search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DIETLYAPI_INTEGRATION_APIKEY (api key), DIETLYAPI_INTEGRATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DIETLYAPI_INTEGRATION_APIKEY carries the API key and
	// DIETLYAPI_INTEGRATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DIETLYAPI_INTEGRATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DIETLYAPI_INTEGRATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDietlyapiIntegrationSDK(opts)
