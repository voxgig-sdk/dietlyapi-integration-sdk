# dietlyapi-integration-cli

boru-driven command-line client **and** interactive REPL for the DietlyapiIntegration
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dietlyapi-integration-cli)
make build

# 2. See usage (words, entities, env vars)
./dietlyapi-integration-cli --help

# 3. Provide credentials once, via the environment
export DIETLYAPI_INTEGRATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./dietlyapi-integration-cli load 1 barcode            # {id:1} shorthand
./dietlyapi-integration-cli load '{id:1}' barcode       # explicit match map

# 5. Override the API base URL for a single call
DIETLYAPI_INTEGRATION_BASE=https://api.example.com ./dietlyapi-integration-cli load 1 barcode

# 6. No arguments -> interactive REPL
./dietlyapi-integration-cli
dietlyapi-integration> load 1 barcode
dietlyapi-integration> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dietlyapi-integration-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dietlyapi-integration-cli load 1 barcode
   ```
...
```sh
./dietlyapi-integration-cli load 1 barcode          # scalar shorthand for {id:1}
./dietlyapi-integration-cli load '{id:1}' barcode     # explicit match map
```
...
./dietlyapi-integration-cli load 1 barcode
```
...
Run with no arguments to open a REPL (prompt `dietlyapi-integration>`). Each line is
evaluated as its own boru expression:

```text
$ ./dietlyapi-integration-cli
dietlyapi-integration> load 1 barcode
dietlyapi-integration> /help
dietlyapi-integration> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `barcode`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dietlyapi-integration-cli`. |
...
(`./dietlyapi-integration-cli <expr>`) and interactively (the REPL), and expressions compose the
