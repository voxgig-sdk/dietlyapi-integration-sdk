# DietlyapiIntegration SDK test runner
...
class DietlyapiIntegrationTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DietlyapiIntegrationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DietlyapiIntegrationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DietlyapiIntegrationTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = DietlyapiIntegrationTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DietlyapiIntegrationTestRunner.getenv(key)
        ...
        explain = DietlyapiIntegrationTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if DietlyapiIntegrationTestRunner._test_control is not None:
            return DietlyapiIntegrationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DietlyapiIntegrationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DietlyapiIntegrationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DietlyapiIntegrationTestRunner.load_test_control()
...
def load_env_local():
    DietlyapiIntegrationTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return DietlyapiIntegrationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DietlyapiIntegrationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DietlyapiIntegrationTestRunner.load_test_control()


def live_client_options():
    return DietlyapiIntegrationTestRunner.live_client_options()


def live_delay_ms():
    return DietlyapiIntegrationTestRunner.live_delay_ms()
