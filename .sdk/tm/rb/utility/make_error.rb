...
module DietlyapiIntegrationUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = DietlyapiIntegrationContext.new({}, nil)
    end
    op = ctx.op || DietlyapiIntegrationOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || DietlyapiIntegrationResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(DietlyapiIntegrationError) ? err.msg : err.to_s
    ...
    sdk_err = DietlyapiIntegrationError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(DietlyapiIntegrationError)
