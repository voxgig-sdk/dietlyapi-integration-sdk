...
// DietlyapiIntegration SDK feature test
//
...
class FtCtrl extends DietlyapiIntegrationControl
{
...
class FtClient
{
    ...
    private DietlyapiIntegrationUtility $utility;

    public function __construct(DietlyapiIntegrationUtility $utility, array $options)
    {
    ...
    public function get_utility(): DietlyapiIntegrationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DietlyapiIntegrationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DietlyapiIntegrationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DietlyapiIntegrationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DietlyapiIntegrationConfig::make_feature($name);
    ...
    private static function buildUrl(DietlyapiIntegrationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DietlyapiIntegrationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DietlyapiIntegrationResponse($fetched);
        }
    ...
    private function fail(DietlyapiIntegrationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DietlyapiIntegrationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
