...
//    only - a typed DietlyapiIntegrationContext there would make every ctx
...
final class DietlyapiIntegrationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new DietlyapiIntegrationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new DietlyapiIntegrationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new DietlyapiIntegrationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new DietlyapiIntegrationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new DietlyapiIntegrationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?DietlyapiIntegrationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DietlyapiIntegrationError(is_string($code) ? $code : '', $msg);
    }
