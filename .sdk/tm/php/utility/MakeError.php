...
class DietlyapiIntegrationMakeError
{
    public static function call(?DietlyapiIntegrationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DietlyapiIntegrationContext([], null);
        }
        $op = $ctx->op ?? new DietlyapiIntegrationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DietlyapiIntegrationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DietlyapiIntegrationError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DietlyapiIntegrationError('', $msg, $ctx);
        ...
        if ($err instanceof DietlyapiIntegrationError) {
