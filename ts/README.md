# DietlyapiIntegration TypeScript SDK



The TypeScript SDK for the DietlyapiIntegration API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Barcode()` — each with a small set of operations (`list`, `load`)
...
```ts
const client = new DietlyapiIntegrationSDK({ apikey: '...' })
...
```ts
new DietlyapiIntegrationSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `DietlyapiIntegrationSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `DietlyapiIntegrationSDK.test(testopts?, sdkopts?)` | `DietlyapiIntegrationSDK` | Create a test-mode client. |
...
| `client` | `client(): DietlyapiIntegrationSDK` | Return the parent SDK client. |
...
Create an instance: `const barcode = client.Barcode()`
...
```ts
const barcode = await client.Barcode().load({ id: 'barcode_id' })
```


### Food

Create an instance: `const food = client.Food()`
...
```ts
const food = await client.Food().load({ id: 1 })
```

#### Example: List

```ts
const foods = await client.Food().list()
```
...
├── src/
│   ├── DietlyapiIntegrationSDK.ts        # Main SDK class
