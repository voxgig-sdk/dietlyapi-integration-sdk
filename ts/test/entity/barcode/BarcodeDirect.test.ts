...
describe('BarcodeDirect', async () => {
  ...
  test('direct-load-barcode', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-barcode', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DIETLYAPI_INTEGRATION_TEST_BARCODE_ENTID']
