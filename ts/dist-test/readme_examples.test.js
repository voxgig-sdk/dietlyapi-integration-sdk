...
const __1 = require("..");
const SDK_NAME = 'DietlyapiIntegrationSDK';
...
const TEST_SEED = { "entity": { "barcode": { "test01": { "id": "test01" } }, "food": { "test01": { "id": "test01" } }, "meta": { "test01": { "id": "test01" } }, "popular": { "test01": { "id": "test01" } }, "search": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+DietlyapiIntegrationSDK\b/.test(code) ||
        /\bDietlyapiIntegrationSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+DietlyapiIntegrationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/DietlyapiIntegrationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
