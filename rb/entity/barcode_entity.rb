...
class BarcodeEntity
  ...
  def make
    opts = @_entopts.dup
    BarcodeEntity.new(@_client, opts)
  end
  ...
  # @return [Barcode, Hash] the current Barcode data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Barcode fields)
  def match_get
  ...
  #
  # @param reqmatch [BarcodeLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Barcode.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Barcode, Hash] the loaded Barcode; raises DietlyapiIntegrationError on failure
