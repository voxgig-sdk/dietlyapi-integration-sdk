...
class FoodEntity
  ...
  def make
    opts = @_entopts.dup
    FoodEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DietlyapiIntegrationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Food, Hash] the current Food data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DietlyapiIntegrationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Food fields)
  def match_get
  ...
  # Load a single Food.
  #
  # @param reqmatch [FoodLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Food.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Food, Hash] the loaded Food; raises DietlyapiIntegrationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DietlyapiIntegrationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [FoodListMatch, Hash, nil] match filter (any subset of
  #   Food fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Food>, Array] the matching Food items; raises DietlyapiIntegrationError on failure
