...
class BarcodeDirectTest < Minitest::Test
  def test_direct_load_barcode
    setup = barcode_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-barcode", setup[:live] ? "live" : "unit")
    if _should_skip
...
def barcode_direct_setup(mockres)
