...
class MetaEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DIETLYAPIINTEGRATION_TEST_META_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def meta_basic_setup(extra)
  ...
  entid_env_raw = ENV["DIETLYAPIINTEGRATION_TEST_META_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "DIETLYAPIINTEGRATION_TEST_META_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["DIETLYAPIINTEGRATION_TEST_META_ENTID"])
