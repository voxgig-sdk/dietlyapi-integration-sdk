...
module DietlyapiIntegrationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DietlyapiIntegrationResponse.new({})
    ...
    if fetch_err
      ...
      response = DietlyapiIntegrationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DietlyapiIntegrationResponse.new(fetched)
    else
