    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DIETLYAPI_INTEGRATION_TEST_BARCODE_ENTID JSON to run live")
        client = setup["client"]
        ...
        barcode_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(barcode_ref01_data_dt0_loaded))
...
def _barcode_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("DIETLYAPI_INTEGRATION_TEST_BARCODE_ENTID"))
