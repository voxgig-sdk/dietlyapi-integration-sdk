...
class TestBarcodeDirect:

    def test_should_direct_load_barcode(self):
        setup = _barcode_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-barcode", "live" if setup["live"] else "unit")
        if _skip:
...
def _barcode_direct_setup(mockres):
