# DietlyapiIntegration Python SDK



The Python SDK for the DietlyapiIntegration API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Barcode()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/dietlyapi-integration-sdk/releases)) or
...
| `serving_desc` | `str` | Human-readable label for one serving, e.g. |
...
| `source` | `str` | Data source: off (Open Food Facts), usda (USDA FoodData Central), claude (labeled AI estimate) or community |
| `static_url` | `str` | Path of the human-readable page on www.getdietly.com |
