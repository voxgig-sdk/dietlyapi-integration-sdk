# dietlyapi-integration-mcp

[MCP](https://modelcontextprotocol.io) server exposing the DietlyapiIntegration SDK as
two agent tools — `dietlyapi-integration_list` and `dietlyapi-integration_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dietlyapi-integration-mcp)
make build

# 2. Provide credentials via the environment
export DIETLYAPI_INTEGRATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dietlyapi-integration \
  -- /absolute/path/to/dietlyapi-integration-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dietlyapi-integration-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dietlyapi-integration_list: first page of records
{ "entity": "food" }
{ "entity": "food", "query": { } }

// dietlyapi-integration_load: one record by id
{ "entity": "barcode", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dietlyapi-integration-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dietlyapi-integration \
     -- "$PWD"/dist/*/dietlyapi-integration-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dietlyapi-integration_list` and `dietlyapi-integration_load` tools now appear
   in new sessions. Ask the agent to *"list food using dietlyapi-integration"*
   and it calls `dietlyapi-integration_list` with `{"entity":"food"}`.
...
```sh
export DIETLYAPI_INTEGRATION_APIKEY=sk_live_xxx            # API key
export DIETLYAPI_INTEGRATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `dietlyapi-integration_list` tool
...
### Call the `dietlyapi-integration_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `dietlyapi-integration_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dietlyapi-integration_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DIETLYAPI_INTEGRATION_APIKEY` | API key sent with every request. |
| `DIETLYAPI_INTEGRATION_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

barcode | food | meta | popular | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./dietlyapi-integration-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dietlyapi-integration_load","arguments":{"entity":"barcode","query":{"id":1}}}}'
```
