...
local BarcodeEntity = {}
BarcodeEntity.__index = BarcodeEntity


function BarcodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BarcodeEntity)
  self._name = "barcode"
  self._client = client
...
function BarcodeEntity:get_name()
  return self._name
end


function BarcodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BarcodeEntity.new(self._client, opts)
end
...
function BarcodeEntity:mark_deleted()
  self._deleted = true
end


function BarcodeEntity:deleted()
  return true == self._deleted
end


function BarcodeEntity:data_set(args)
  if args ~= nil then
...
function BarcodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BarcodeEntity:match_set(args)
  if args ~= nil then
...
function BarcodeEntity:match_get()
...
function BarcodeEntity:stream(action, args, callopts)
...
---@param reqmatch BarcodeLoadMatch
---@param ctrl? table
---@return Barcode
---@return string? err
function BarcodeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function BarcodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
