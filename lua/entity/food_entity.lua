...
function FoodEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FoodEntity)
  self._name = "food"
  self._client = client
...
function FoodEntity:get_name()
  return self._name
end


function FoodEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FoodEntity.new(self._client, opts)
end
...
function FoodEntity:mark_deleted()
  self._deleted = true
end


function FoodEntity:deleted()
  return true == self._deleted
end


function FoodEntity:data_set(args)
  if args ~= nil then
...
function FoodEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FoodEntity:match_set(args)
  if args ~= nil then
...
function FoodEntity:stream(action, args, callopts)
...
---@param reqmatch FoodLoadMatch
---@param ctrl? table
---@return Food
---@return string? err
function FoodEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch FoodListMatch
---@param ctrl? table
---@return Food[]
---@return string? err
function FoodEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function FoodEntity:_run_op(ctx, post_done)
  local utility = self._utility
