# DietlyapiIntegration Lua SDK



The Lua SDK for the DietlyapiIntegration API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Barcode()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/dietlyapi-integration-sdk/releases)),
...
Create an instance: `local barcode = client:Barcode(nil)`
...
| `barcode` | `string` |  |
| `brand` | `string` |  |
| `calories_kcal` | `number` | Energy in kcal per 100 g |
| `carbs_g` | `number` | Carbohydrate in grams per 100 g |
| `category` | `string` |  |
...
| `image_url` | `string` |  |
| `name` | `string` |  |
| `potassium_mg` | `number` | Potassium in mg per 100 g |
| `protein_g` | `number` | Protein in grams per 100 g |
| `saturated_fat_g` | `number` | Saturated fat in grams per 100 g |
| `serving_desc` | `string` | Human-readable label for one serving, e.g. |
| `serving_size_g` | `number` | Grams in one manufacturer serving, where the source declares one. |
| `sodium_mg` | `number` | Sodium in mg per 100 g |
...
| `sugar_g` | `number` | Sugars in grams per 100 g |

#### Example: Load

```lua
local barcode, err = client:Barcode():load({ id = "barcode_id" })
```


### Food

Create an instance: `local food = client:Food(nil)`
...
```lua
local food, err = client:Food():load({ id = 1 })
```

#### Example: List

```lua
local foods, err = client:Food():list()
```
...
The main module (`dietlyapi-integration_sdk`) exports the SDK constructor
